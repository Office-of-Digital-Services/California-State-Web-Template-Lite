...
The Template Link Grid component displays a set of prominent links as bordered cards in a responsive grid. It uses the `<catmplt-link-grid>` root class and strict direct-child selectors, so the HTML structure must remain consistent.
...
- The root wrapper must be `<catmplt-link-grid>`.
...
- Link styles are applied to `<catmplt-link-grid>`.
