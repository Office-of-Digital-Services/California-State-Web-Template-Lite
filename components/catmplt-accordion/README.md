...
The Template Accordion component is a progressively enhanced disclosure pattern built on native `details` and `summary` elements. It features a custom-tag wrapper architecture (`<catmplt-accordion>`) combined with structural descendant selectors and a lightweight JavaScript controller for smooth height animation and optional single-open behavior.
...
Instead of utility classes, this component uses a single `data-open` attribute on the root section element to control accordion interaction behavior.

### Open Behavior (`data-open`)

Applied to the root `section` element.
